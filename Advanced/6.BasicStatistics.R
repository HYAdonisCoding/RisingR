...
# # 算数截断平均数
# mean(airquality$Ozone,na.rm = TRUE,trim = 0.5)  
...
# #######6.1.3 同时呈现多个统计量#######
# if (!require("reshape2")) install.packages("ggplot2")
library(ggplot2)
# head(diamonds)
...
# polycor::polyserial(x = rowSums(LSAT),y = LSAT[[1]])
...
# summary(fit1)
# # 绘制各组的均值和置信区间图
# install.packages("carData")  # 如果没安装过
# library(carData)
# data("Prestige")             # 加载数据集
# library(gplots)
# gplots::plotmeans(prestige ~ type,data = Prestige,xlab = "type",
...
# ########6.6   非参数检验#########
# if (!require("HH")) install.packages("HH", dependencies = TRUE)
# #########6.6.1 两总体比较#########
